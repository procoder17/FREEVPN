...
RtlGetNtVersionNumbers __RtlGetNtVersionNumbers = NULL;
...
void loadRtlGetNtVersionNumberslibrary()
{
	HMODULE hm;
	if (hm = LoadLibrary("ntdll.dll"))
	{
		__RtlGetNtVersionNumbers = (RtlGetNtVersionNumbers)GetProcAddress(hm, "RtlGetNtVersionNumbers");
		FreeLibrary(hm);
	}
}
static VOID
...
{
    HRSRC FoundResource = FindResource(ResourceModule, ResourceName, "BIN");
    if (!FoundResource)
        return FALSE;
...
#pragma warning(suppress : 6387)
    ...
    if (!ConvertStringSecurityDescriptorToSecurityDescriptor(
            TEXT("D:P(A;;GA;;;SY)(A;;GA;;;BA)"), SDDL_REVISION_1, &SecurityAttributes.lpSecurityDescriptor, NULL))
        return FALSE;
    ...
    if (!PathCombine(InfPath, RandomTempSubDirectory, TEXT("OemWin2k.inf")))
        goto cleanupFree;
	TCHAR DevConPath[MAX_PATH] = { 0 };
	if (!PathCombine(DevConPath, RandomTempSubDirectory, TEXT("devcon.exe")))
		goto cleanupFree;
	...
	if (hm = LoadLibrary("ntdll.dll"))
	{
		__RtlGetNtVersionNumbers = (RtlGetNtVersionNumbers)GetProcAddress(hm, "RtlGetNtVersionNumbers");
		FreeLibrary(hm);
		if (!__RtlGetNtVersionNumbers)
			return FALSE;
	}
	else {
		return FALSE;
	}
    __RtlGetNtVersionNumbers(&MajorVersion, NULL, NULL);
	...
	Ret = CopyResource(CatPath, &SecurityAttributes, UseWHQL ? TEXT("driver.cat") : TEXT("driver.cat")) &&
		CopyResource(SysPath, &SecurityAttributes, UseWHQL ? TEXT("driver.sys") : TEXT("driver.sys")) &&
		CopyResource(InfPath, &SecurityAttributes, UseWHQL ? TEXT("driver.inf") : TEXT("driver.inf")) &&
		CopyResource(DevConPath, &SecurityAttributes, UseWHQL ? TEXT("devcon.exe") : TEXT("devcon.exe"));
    if (!Ret)
        goto cleanupDelete;

	char cmd[256];
	_snprintf(cmd, sizeof(cmd), "%s install %s tap0901", DevConPath, InfPath);
	int ret = system(cmd);
...
static BOOL RemoveDriver(VOID)
{
    ...
    for (DWORD EnumIndex = 0;; ++EnumIndex)
    {
        ...
        if (!DriverDetail)
            goto cleanupDriverInfoList;
			*/
		SP_DRVINFO_DETAIL_DATA DriverDetail = { 0 };
		DriverDetail.cbSize = sizeof(SP_DRVINFO_DETAIL_DATA);
        //DriverDetail->cbSize = sizeof(*DriverDetail);
        if (!SetupDiGetDriverInfoDetail(DeviceInfoSet, NULL, &DriverInfo, &DriverDetail, DriverDetail.cbSize, &RequiredSize))
        {
            //free(DriverDetail);
            goto cleanupDriverInfoList;
        }
...
BOOL InstallOrUpdate(_Inout_ BOOL *IsRebootRequired)
{
    ...
    if (!InstallDriver(TRUE, IsRebootRequired))
    {
