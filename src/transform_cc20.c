...
//       |<---- encrypted ---->|
//
int transop_encode_cc20 (n2n_trans_op_t *arg,
                                uint8_t *outbuf,
                                size_t out_len,
                                const uint8_t *inbuf,
                                size_t in_len,
    ...
    if(in_len <= N2N_PKT_BUF_SIZE) {
        if((in_len + CC20_PREAMBLE_SIZE) <= out_len) {
	        ...
	        encode_uint64(outbuf, &idx, rand());
            encode_uint64(outbuf, &idx, rand());

            len = in_len;

            cc20_crypt(outbuf + CC20_PREAMBLE_SIZE, inbuf, in_len, outbuf, /* iv */ priv->ctx);
...
static int setup_cc20_key (transop_cc20_t *priv, const uint8_t *password, ssize_t password_len) {
	...
	Sha2_256(key_mat, (void*)password, password_len);
...
static n2n_tostat_t transop_tick_cc20 (n2n_trans_op_t *arg, long now) {
...
int n2n_transop_cc20_init (const u_char*encrypt_key, n2n_trans_op_t *ttt) {
    ...
    transop_cc20_t *priv;
    size_t encrypt_key_len = strlen((const char*)encrypt_key);
