...
static int setup_ifname (tuntap_dev* device, int fd, const char *ifname, const char *ipaddr,
...
int tuntap_open (tuntap_dev *device,
    ...
    memset(&ifr, 0, sizeof(ifr));
    ifr.ifr_flags = (((device->tuntap_mode == TUN_MODE) ? IFF_TUN : IFF_TAP) | IFF_NO_PI);
    ...
    if(setup_ifname(device, ioctl_fd, device->dev_name, device_ip, device_mask, device->mac_addr, mtu) < 0) {
        close(nl_fd);
        close(ioctl_fd);
        close(device->fd);
        return -1;
